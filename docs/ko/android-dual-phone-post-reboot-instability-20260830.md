...
사용자는 임시 wake lock을 켠 뒤 현재까지 PocketRisu 서버를 계속 실사용하고 있으며, 이전처럼 수 분 내 완전 단절되는 현상이 재현되지 않고 정상 사용이 이어지고 있다고 보고했습니다.

따라서 앞선 15분 스크립트의 tunnel PID 검증은 잘못된 실행 위치/경로 때문에 유효하지 않았지만, **wake lock 이후 실제 사용 중 연결 안정성이 개선된 관찰 자체는 유효한 별도 증거**로 유지합니다. 이전 상태에서는 wake lock 없이 일정 시간이 지나면 direct SSH 8022가 `Connection refused`로 바뀌고 Termux를 열면 runit 전체가 약 2초 age로 재기동되는 현상이 반복됐습니다. 반면 임시 wake lock 활성화 이후에는 사용자가 계속 정상 사용 중입니다.

이 차이는 wake lock이 서버폰 Termux background 생존성에 실질적으로 도움을 주고 있을 가능성을 강하게 높이지만, 아직 통제된 메인폰 원격 A/B가 아니므로 인과를 100% 확정하지는 않습니다. 현재 안정 상태를 깨지 않기 위해 임시 wake lock은 유지하고, 추가 진단은 필요 최소한으로 진행합니다.

## 반복 관찰: wake lock 구간에서는 동일 장애 미발생

사용자는 이전 운영 경험까지 포함해 **wake lock을 유지한 구간에서는 이번과 같은 완전 단절/runit 전체 재기동 현상을 한 번도 경험하지 않았다**고 추가로 보고했습니다. 반대로 wake lock을 부팅 초기화 후 해제하도록 바꾼 뒤에는 일정 시간이 지나 서버 8022가 `Connection refused`가 되고, Termux를 다시 열면 runit 아래 서비스 전체가 짧은 age로 재기동되는 현상이 실제로 관찰됐습니다.

이는 통제된 단일 실험은 아니지만, 반복된 실사용 대비 관찰이라는 점에서 단순 우연보다 더 강한 증거입니다. 현재 운영 가설 우선순위는 **서버폰의 지속 wake lock 유지가 Termux/runit background 생존성에 핵심적으로 기여한다**는 쪽으로 상향합니다. 다만 Android 프로세스 생존과 CPU wake lock의 직접 인과 메커니즘은 별도 검증 전까지 확정 표현을 피합니다.
