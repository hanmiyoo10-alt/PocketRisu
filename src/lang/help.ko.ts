...
export const helpKo = {
        ...
        "allowV21Plugin": "경고: V2.1 API는 비권장 상태이며 향후 버전에서 동작이 중단될 예정이라 새 V2.1 플러그인 설치가 기본적으로 차단됩니다. 이미 설치된 V2.1 플러그인은 계속 동작합니다. 신뢰하는 V2.1 플러그인을 설치·업데이트해야 할 때만 켜세요. V2.1 안전성 검사는 그대로 적용됩니다.",
        ...
        "memoryPresetBinding": "이 채팅이 사용할 장기기억 프리셋입니다.\n\n- **기본값**: 설정 → 장기기억의 기본 프리셋을 따릅니다. 기본 프리셋을 바꾸면 함께 바뀝니다.\n- **사용 안 함**: 컨텍스트를 넘어선 과거 메시지는 단순히 잘려 모델에 전달되지 않습니다.\n- **프리셋 선택**: 기본값과 무관하게 이 채팅만 해당 프리셋(요약 모델·비율 등)으로 동작합니다.\n\n채팅 단위로 저장되므로 같은 캐릭터의 다른 채팅에는 영향을 주지 않습니다.",
        "memoryPresetDefault": "새로 켜는 채팅과 '기본값'으로 둔 채팅이 사용할 장기기억 프리셋입니다. '사용 안 함'이면 채팅에서 직접 프리셋을 고른 경우에만 장기기억이 동작합니다.",
        ...
        "nodeOnlyRestoreLastChat": "앱을 열거나 새로고침할 때 마지막으로 보던 캐릭터의 채팅을 다시 엽니다. 모바일 브라우저가 백그라운드에서 탭을 종료해도 채팅으로 돌아옵니다. 홈에서 나갔다면 홈이 유지됩니다.",
        ...
        "hideRecentChats": "캐릭터를 선택하지 않았을 때 사이드바에 표시되는 최근 대화 목록을 숨깁니다.",
        "hideDeactivatedCharacters": "비활성화된 캐릭터를 사이드바 아이콘 목록에서 흐리게 표시하는 대신 아예 숨깁니다. 캐릭터 관리와 스토리지 대시보드에는 계속 표시됩니다.",
